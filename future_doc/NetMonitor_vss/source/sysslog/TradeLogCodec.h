...
*/
class CSlogSouceKey;
class CSlogSouceShortKey
{
	/* 根据CSlogSouceKey可以映射到唯一的shortkey */
	/* 原来需要使用30+个字节表示的SourceKey，现在只需要7个字节 */
public:
	// 时间戳
	int m_nTime;
	// 源ID，根据IP+Member+User得到的唯一ID
	short m_nSourceID;
	// 使用一个字符描述FrontID，使用int浪费空间
	// SessionID == FrontID
	unsigned char m_nFrontID;
public:
	CSlogSouceShortKey();
	~CSlogSouceShortKey();

	CSlogSouceShortKey(CSlogSouceKey _sourceKey);

	bool operator>(const CSlogSouceShortKey &ob) const;
...
public:
	// 将此shortkey转换为sourcekey
	CSlogSouceKey getSourceKey() const;

private:
	// IP+Member+User列表
	// 其他字段为空
	static vector<CSlogSouceKey> m_sourceArray;
};
