...
class COrderRttAnalysis
{
	...
	private:
		...
		static CQWordMonitorIndex* m_TotalRttCountMonitor;
	/* */
	public:
		///OFP数据包处理函数
		...
		{
			if (m_TotalRttCountMonitor)
			{
				m_TotalRttCountMonitor->setValue(0);
			}
			else
			{
				m_TotalRttCountMonitor = new CQWordMonitorIndex("TotalRttCount", 10);
			}
		}
