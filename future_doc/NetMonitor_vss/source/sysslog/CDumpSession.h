...
class CSlogQuery
{
	...
	public:
		...
		///构造函数
		CSlogQuery(void) :m_nMallocSize(0)
		{
			m_pkgSend.ConstructAllocate(FTDC_PACKAGE_MAX_SIZE, 512);
			if (m_pSlogMallocLengthMonitor)
			{
				m_pSlogMallocLengthMonitor->setValue(0);
			}
			else
			{
				m_pSlogMallocLengthMonitor = new CQWordMonitorIndex("SlogMallocLength", 5);
			}
		}
		...
		void print(void)
		{
				 ...
				 it != m_mapSlog.end();)
			{
				...
				printf(
					"[%s][%s][%s][%s][%s]\n",
					_slogSourceKey.m_sTime.c_str(),
					_slogSourceKey.m_sIpAddr.c_str(),
					_slogSourceKey.m_sMemberId.c_str(),
					_slogSourceKey.m_sUserId.c_str(),
					it->second);
				++it;
			}
		}
	...
	/* */
	private:
		bool checkMemory(int nLength);

		void eraseFrontElements();
	...
	/* */
	private:
		typedef multimap<CSlogSouceShortKey, char *> SLOGSOURCEMAP;
		...
		///统计存储包的内存大小，防止内存分配过大溢出
		unsigned long long m_nMallocSize;

		static CQWordMonitorIndex* m_pSlogMallocLengthMonitor;
};
