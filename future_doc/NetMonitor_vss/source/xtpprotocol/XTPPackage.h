...
struct TXTPExtHeader : public TFieldHeader
{
	DWORD FrontSubjectID;	/**< 前置主题代码*/
	DWORD FrontSeqNo;		/**< 在前置主题中的序号*/
	DWORD CompSeqNo;		/**< 排队机产生的序号*/

	DWORD FrontRecvSec;		/**< 前置产生报文的时刻（秒）*/
	DWORD FrontRecvUsec;	/**< 前置产生报文的时刻（微秒）*/
	DWORD CompUseTime;		/**< 排队耗时（微秒）*/
	DWORD TkernelUseTime;	/**< 业务计算耗时（微秒）*/
...
inline void TXTPExtHeader::ChangeEndian()
{
	CHANGE_ENDIAN(FieldID);
	CHANGE_ENDIAN(Size);

	CHANGE_ENDIAN(FrontSubjectID);
	CHANGE_ENDIAN(FrontSeqNo);
	CHANGE_ENDIAN(CompSeqNo);

	CHANGE_ENDIAN(FrontRecvSec);
	CHANGE_ENDIAN(FrontRecvUsec);
	CHANGE_ENDIAN(CompUseTime);
	CHANGE_ENDIAN(TkernelUseTime);
}

const int  XTPEXTHLEN	= sizeof(TXTPExtHeader);

const WORD FID_XTP_EXT_HEADER = 0xF701;

inline void TXTPExtHeader::Init()
{
	memset(this, 0 ,sizeof(TXTPExtHeader));
	FieldID = FID_XTP_EXT_HEADER;
}

inline void TXTPExtHeader::Enable()
{
	Size = XTPEXTHLEN-sizeof(TFieldHeader);
}

inline void TXTPExtHeader::Disable()
{
	Size = 0;
}

inline bool TXTPExtHeader::IsEnable()
{
	return Size > 0;
}

inline void TXTPExtHeader::ToStream(char *pStream)
{
	memcpy(pStream, this, sizeof(TXTPExtHeader));
	((TXTPExtHeader *)pStream)->ChangeEndian();
}

inline void TXTPExtHeader::FromStream(char *pStream)
{
	memcpy(this, pStream, sizeof(TXTPExtHeader));
	ChangeEndian();
...
public:
	...
	*/
	...
	inline TXTPExtHeader *GetExtHeader();
	...
	*/
	...
	/**
	 * 把头的长度加入到缓存区，如果有扩展头同时处理了扩展头
	 */
	void ForcePushHeader();

	/**
	 * 把头的长度移出缓存区，如果有扩展头同时处理了扩展头
	 */
	void ForcePopHeader();
...
inline TXTPExtHeader *CXTPPackage::GetExtHeader()
{
	return &m_extHeader;
}
