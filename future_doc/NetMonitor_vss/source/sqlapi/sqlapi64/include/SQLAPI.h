...
struct SAStringData
{
	...
	long nDataLength;		// length of data (including terminator)
        long nAllocLength;		// length of allocation
...
class SQLAPI_API SACommand
{
	...
	bool			m_bParamsKnown;
	bool                    m_x,m_y,m_z; //for compact pack
...
public:
	...
	void setParamPrecision(int precision);
	int ParamScale() const;
	void setParamScale(int scale);
