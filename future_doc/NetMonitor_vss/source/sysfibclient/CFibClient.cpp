...
void CFibClient::CreateSubscriber(WORD wTopicID)
{
...
void CFibClient::ResetSubscriber(void)
{
	//清除proxy流水
...
CSession *CFibClient::CreateSession(CChannel *pChannel, DWORD dwMark)
{
	//printf("运行文件 ： %s 行数：%d \n", __FILE__, __LINE__);
...
void CFibClient::OnSessionConnected(CSession *pSession)
{
	time_t	timep;
	time(&timep);
	printf(
		"\t %s CFibClient::OnSessionConnected: %0x \n",
		ctime(&timep),
		pSession);
...
void CFibClient::OnSessionDisconnected(CSession *pSession, int nReason)
{
	time_t	timep;
	time(&timep);
	printf(
		"\t %s CFibClient::OnSessionDisconnected: %0x \n",
		ctime(&timep),
		pSession);
...
void CFibClient::OnTimer(int nIDEvent)
{
...
int CFibClient::RequestDirectly(void)
{
	NET_IO_LOG1(
...
void CFibClient::SetRetryTimer(void)
{
	KillTimer(TIMER_CHECK_RESPONSE);
...
void CFibClient::ReqSubscribe(void)
{
	m_pkgRequest.PreparePackage(
...
void CFibClient::OnRspError(CFTDCPackage *pFTDCPackage, CFTDCSession *pSession)
{
...
void CFibClient::OnRspUserLogin(
	CFTDCPackage *pFTDCPackage,
	CFTDCSession *pSession)
{
	//printf("运行文件 ： %s 行数：%d \n", __FILE__, __LINE__);
...
void CFibClient::OnRspSubscribe(
	CFTDCPackage *pFTDCPackage,
	CFTDCSession *pSession)
{
	CFPPRspInfoField fieldRspInfo;
